...
function SwordMark() {
  return (
    <svg viewBox="0 0 80 80" fill="none" aria-hidden className="h-20 w-20">
      ...
      <path d="m27 42 11 11-6 6-11-11 6-6Z" className="fill-raid-bg" opacity=".9" />
      <path d="m22 55 4 4-12 12-4-4 12-12Z" fill="currentColor" />
      <path d="M55 14 64 11l-2 9-25 25-7-7 25-24Z" className="fill-raid-bg" opacity=".28" />
    </svg>
  );
}
  ...
  return (
    <div className="relative mx-auto w-full max-w-6xl flex-1 overflow-hidden px-5 py-10 sm:py-14">
      ...
      <section className="mt-10 grid gap-5 lg:grid-cols-[1.45fr_0.55fr]" aria-label="Typing games">
        <Link
          href="/games/typeraid"
          className="group relative min-h-[28rem] overflow-hidden rounded-3xl border border-[color-mix(in_srgb,var(--accent)_35%,var(--border))] bg-raid-surface p-6 text-raid-text shadow-[0_28px_80px_-42px_color-mix(in_srgb,var(--accent)_75%,transparent)] transition duration-300 hover:-translate-y-1 hover:border-[color-mix(in_srgb,var(--accent)_65%,var(--border))] sm:p-9"
        >
          <div aria-hidden className="absolute inset-0 bg-[radial-gradient(circle_at_78%_24%,color-mix(in_srgb,var(--accent)_24%,transparent),transparent_30%),radial-gradient(circle_at_18%_90%,rgba(255,91,137,.12),transparent_32%),linear-gradient(135deg,var(--raid-fill),transparent_42%)]" />
          <div aria-hidden className="absolute -right-10 top-12 h-64 w-64 rotate-12 rounded-[35%_65%_55%_45%] border border-raid-border bg-raid-fill transition-transform duration-700 group-hover:rotate-[22deg] group-hover:scale-110" />

          <div className="relative flex h-full min-h-[24rem] flex-col">
            <div className="flex items-start justify-between gap-4">
              <span className="rounded-full border border-raid-border bg-raid-fill px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-raid-muted">
                Playable now
              </span>
              <div className="text-raid-accent transition-transform duration-300 group-hover:-rotate-6 group-hover:scale-110">
                <SwordMark />
              </div>
            </div>

            <div className="mt-auto max-w-xl">
              <p className="font-mono text-xs text-raid-accent">01 / dungeon run</p>
              <h2 className="mt-2 text-5xl font-bold tracking-[-0.06em] sm:text-6xl">TypeRaid</h2>
              <p className="mt-4 max-w-md text-sm leading-6 text-raid-muted">
              ...
              <div className="mt-7 flex flex-wrap items-center gap-3">
                <span className="rounded-xl bg-raid-text px-5 py-2.5 text-sm font-semibold text-raid-bg transition-transform group-hover:translate-x-1">
                  Enter the raid →
                </span>
                <span className="text-xs text-raid-faint">4 encounters · 1 final boss · endless replay</span>
              </div>
            </div>
          </div>
        </Link>
